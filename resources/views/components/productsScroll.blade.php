...
<div class="flex-col flex relative  max-w-[100vw]">
    ...
    <div class="relative flex overflow-y-hidden pt-1 overflow-x sm:overflow-x-hidden self-center scroll-smooth snap-x scrollable w-full shadow-sm">
                    ...
                    <a class="flex flex-col px-4 h-full" href="/product/{{ $product->id }}">
                        ...
                        @if (Auth::check() && Auth::getUser()->type == 'trader')
                            @if ($product->wholesale)
                                <div class="z-10 text-sm font-semibold mr-1">Wholesale: {{ number_format($product->wholesale) }} <span class="text-[.83rem]">EGP</span></div>
                            @else
