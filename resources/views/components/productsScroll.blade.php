@if (count($products) > 0)
    ...
    <div class="flex-col flex relative  w-full ">
        <button class="h-full w-10 flex absolute z-10 scroll-arrow cursor-pointer left-arrow rotate-180">
            <img class="h-6 w-6 m-auto" src="{{ Vite::asset('resources/images/right-arrow.svg') }}" alt="">
        </button>
        <button class="h-full w-10 flex absolute z-10 scroll-arrow cursor-pointer right-arrow right-0">
            <img class="h-6 w-6 m-auto" src="{{ Vite::asset('resources/images/right-arrow.svg') }}" alt="">
        </button>
            ...
            @foreach ($products as $product)
                <a class="scroll-img w-[50vw] lg:w-[16.66vw] sm:w-[33.3vw] px-1 flex-shrink-0" id="b02" href="/product/{{ $product->id }}">
                    <div class="relative flex flex-col h-full border-[1px] rounded-md border-gray-300">
                        ...
                        @endif
                        <img src="{{ $fileUrl . '/file/connect-store/product/' . $product->img_id. '/0'. '?Authorization=' . $fileToken . '&b2ContentDisposition=attachment' }}" class="object-contain  rounded  -translate-y-0 h-64" />
